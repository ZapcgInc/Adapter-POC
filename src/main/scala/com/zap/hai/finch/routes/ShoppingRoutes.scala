...
//
//
//    val parameters = ArrayBuffer[(String, String)](("checkin", checkin), ("checkout", checkout), ("currency", currency), ("language", language),
//      ("country_code", countryCode), ("occupancy", occupancy), ("sales_channel", salesChannel), ("sales_environment", salesEnvironment),
//      ("sort_type", sortType))
//
//    propertyId.map(e => parameters += (("property_id", e)))
//    filter.map(e => parameters += (("filter", e)))
//    include.map(e => parameters += (("include", e)))
//    rateOption.map(e => parameters += (("rate_option", e)))
//    billingTerms.map(e => parameters += (("billing_terms", e)))
//    paymentTerms.map(e => parameters += (("payment_terms", e)))
//    partnerPos.map(e => parameters += (("partner_point_of_sale", e)))

    //TODO add headers


//    val response = shoppingController.getPropertyAvailability(parameters)
//
